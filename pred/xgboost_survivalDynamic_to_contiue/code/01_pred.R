...
colnames(type.splie) <- paste0('window', 1:ncol(type.splie))
write.csv(type.splie, paste0(pdir, 'window_sample_size.csv'))
...
for (window in 4:16){
  ...
  get_fealist <- function(feature.type = 'gene'){
    ...
    for (j in 1:5) {
       ...
       set.seed(j)
       if (length(id1)<4){
         leaveid <- c(sample(pbmc.s[id1], length(id1)/2), sample(pbmc.s[id2], length(id2)/4))
       } else {

...
## --------------------------
meta <- readRDS('/dcl02/hongkai/data/covid/data/old/200916/meta.rds')
cellProp.mat <- readRDS('/dcs01/gcode/zji4/covid/old/analysis/pbmc/cluprop/fullprop/pbmc.rds')
cyto <- readRDS('/dcl02/hongkai/data/covid/data/old/200916/cytokine_receptor.rds')
cytokine.v <- unique(as.character(cyto[,1]))
receptor.v <- unique(as.vector(apply(cyto[,2:ncol(cyto)], 2, as.character)))
# tf <- fread('/dcl02/hongkai/data/whou/resource/tflist/human/Homo_sapiens_TF', data.table = F)
# tf.v <- intersect(tf[,2], rownames(d[[1]]))
tf <- readRDS('/dcl02/hongkai/data/covid/temp/TF/cistrome_allTFs.rds')
tf <- as.character(tf[!is.na(tf[,2]),1]) ## only use 1041 TFs with target genes 
tf.v <- intersect(tf, rownames(d[[1]])) ## 965 TFs
...
for (arg in list(c('HD','Mi'), c('HD', 'Se'), c('Se', 'Mi'))){
    ...
    for (j in 1:length(unique(as))) {
      ...
      fea <- sapply(setdiff(1:length(d), c(28, 30)), function(cluid){
        ...
        if (feature.type == 'gene'){
          ...
          mat <- mat[rownames(mat) %in% tf.v, colnames(mat) %in% pbmc.s]
        }
  ...
  # saveRDS(fealist.gene, paste0('/dcl02/hongkai/data/whou/covid/pred/xgboost/res/leave_one_study_out_selected_features_gene_', arg[1], '_', arg[2],'.rds'))
  fealist.gene <-  readRDS(paste0('/dcl02/hongkai/data/whou/covid/pred/xgboost/res/leave_one_study_out_selected_features_gene_', arg[1], '_', arg[2],'.rds'))
  
  # ------------------
  ## select features for cytokine 
  # fealist.cyt <- get_fealist(feature.type = 'cytokine')
  # saveRDS(fealist.cyt, paste0('/dcl02/hongkai/data/whou/covid/pred/xgboost/res/leave_one_study_out_selected_features_cytokine_', arg[1], '_', arg[2],'.rds'))
  fealist.cyt <- readRDS(paste0('/dcl02/hongkai/data/whou/covid/pred/xgboost/res/leave_one_study_out_selected_features_cytokine_', arg[1], '_', arg[2],'.rds'))
  
  # --------------------
  ## select features for receptor
  # fealist.rec <- get_fealist(feature.type = 'receptor')
  # saveRDS(fealist.rec, paste0('/dcl02/hongkai/data/whou/covid/pred/xgboost/res/leave_one_study_out_selected_features_receptor_', arg[1], '_', arg[2],'.rds'))
  fealist.rec <-  readRDS(paste0('/dcl02/hongkai/data/whou/covid/pred/xgboost/res/leave_one_study_out_selected_features_receptor_', arg[1], '_', arg[2],'.rds'))
  
  fealist.tf <- get_fealist(feature.type = 'tf')
  saveRDS(fealist.tf, paste0('/dcl02/hongkai/data/whou/covid/pred/xgboost/res/leave_one_study_out_selected_features_tf_', arg[1], '_', arg[2],'.rds'))
  ...
  getauc <- function(base = TRUE,  cytokine = FALSE, receptor = FALSE, tf = FALSE, cellProp = FALSE, gene = FALSE){
    ...
    for (j in 1:length(unique(as))) {
      ...
      invisible(capture.output(model <- xgboost(data = t(mat[, trainid, drop=F]), label = trans(y), nrounds = 20, objective='multi:softprob',num_class = 3)))
  ...
  pdf(paste0('/dcl02/hongkai/data/whou/covid/pred/xgboost/plot/AUC_', arg[1],'_',arg[2], '_boxplot.pdf'), width = 8, height = 4) 
...
pdf('/dcl02/hongkai/data/whou/covid/pred/xgboost/plot/AUC_combine.pdf', width = 22, height = 5) 
...
ddir <- '/dcl02/hongkai/data/whou/covid/pred/xgboost/res/'
allf <- list.files("/dcl02/hongkai/data/whou/covid/pred/xgboost/res")
...
pdf('/dcl02/hongkai/data/whou/covid/pred/xgboost/plot/number_of_features.pdf', width = 7, height = 4.5)
ggplot(data = pd) + 
...
pdf('/dcl02/hongkai/data/whou/covid/pred/xgboost/plot/number_of_unique_features.pdf', width = 7, height = 4.5)
ggplot(data = pd) + 
  geom_boxplot(aes(x = type, y = number)) + 
  geom_jitter(aes(x = type, y = number, color = study)) +
  theme_classic() +

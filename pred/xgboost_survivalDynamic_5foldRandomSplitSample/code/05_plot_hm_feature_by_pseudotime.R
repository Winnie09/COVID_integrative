...
for (window in 10:16){
  ...
  for (featureType in c('gene', 'cytokine', 'receptor', 'tf')){
    feamat <- mat[impt[impt[,1] == paste0('window', window) & impt[,2] == featureType, 3], ]
    str(feamat)
    feamat.fit <- t(sapply(1:nrow(feamat), function(i){
      id <- !is.na(feamat[i,])
          tmp <- loess(feamat[i,]~seq(1,ncol(feamat)))$fitted
          v <- rep(NA, ncol(feamat))
          v[id] <- tmp
          v
        }))
    dimnames(feamat.fit) <- dimnames(feamat)
    p <- mySTIP2(feamat.fit, gl = rownames(feamat.fit))
    ggsave(paste0(pdir, 'hm_feature_window', window,'_', featureType, '.png'),p, width = 6, height = 12, dpi = 200)
  }
}

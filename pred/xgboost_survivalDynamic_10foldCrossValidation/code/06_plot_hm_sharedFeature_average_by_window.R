...
feamat <- sapply(10:16, function(window){
  ...
  mat.add <- t(d.pt[colnames(mat), c('Age', 'Sex')])
  mat.add[2,] <- as.numeric(ifelse(mat.add[2,] == 'M', 0, 1))
  mat.add2 <- matrix(as.numeric(mat.add), nrow = nrow(mat.add))
  dimnames(mat.add2) <- dimnames(mat.add)
  mat <- rbind(mat, mat.add2)
...
ggsave(paste0(pdir, 'hm_feature_window_average.png'),p, width = 6, height = 30, dpi = 200)


fea.share
feamat <- t(sapply(fea.share, function(i){
  clu = paste0('c', sub('cluster', '', sub(';.*', '', i)))
  g = sub('.*;', '', i)
  v <- rep(NA, nrow(d.pt))
  names(v) <- rownames(d.pt)
  tmp <- d[[clu]][g, colnames(d[[clu]]) %in% names(v)]
  v[names(tmp)] <- tmp
  v
}))
feamat.fit <- t(sapply(1:nrow(feamat), function(i){
  mod <- loess(feamat[i,]~seq(1,ncol(feamat)))
      tmp <- predict(object = mod, newdata = seq(1,nrow(d.pt)))
    }))
dimnames(feamat.fit) <- dimnames(feamat)
p <- mySTIP2(feamat.fit, gl = rownames(feamat.fit))
ggsave(paste0(pdir, 'hm_feature_along_pseudotime.png'),p, width = 6, height = 4.5, dpi = 200)

p <- mySTIP2(feamat, gl = rownames(feamat.fit))
ggsave(paste0(pdir, 'hm_feature_along_pseudotime_ori.png'),p, width = 6, height = 4.5, dpi = 200)

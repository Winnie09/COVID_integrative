...
for (window in 10:16){
  ...
  set.seed(12345)
  leaveid.list1 <-
    split(sample(pbmc.s[id1]), ceiling(seq_along(pbmc.s[id1]) / (length(id1) / 10)))
  leaveid.list2 <-
    split(sample(pbmc.s[id2]), ceiling(seq_along(pbmc.s[id2]) / (length(id2) / 10)))
  leaveid.list <- lapply(1:length(leaveid.list1), function(i) {
    c(leaveid.list1[[i]], leaveid.list2[[i]])
  })
  saveRDS(leaveid.list,
          paste0(rdir, 'leaveid.list_', 'window', window, '.rds'))
  ...
  get_fealist <- function(feature.type = 'gene'){
    ...
    for (j in 1:10) {
       ...
       if (length(table(d.pt[leaveid,'Clinical.Outcome'])) == 2 & length(table(d.pt[trainid,'Clinical.Outcome'])) == 2){
  ...
  getauc <- function(base = TRUE,  cytokine = FALSE, receptor = FALSE, tf = FALSE, cellProp = FALSE, gene = FALSE){
    ...
    for (j in 1:length(allfealist)) {
      ...
      mat <- rbind(mat, pseudotime = as.numeric(d.pt[colnames(mat),'Pseudotime']))

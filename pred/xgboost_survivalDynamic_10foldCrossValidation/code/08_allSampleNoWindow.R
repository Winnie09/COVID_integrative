...
d.pt <-
  readRDS('/Users/wenpinhou/Dropbox/covid/pred/data/data/sample_info_pb_opt.rds')
rownames(d.pt) <- d.pt$Patient
str(d.pt)
d.pt <-
  d.pt[d.pt[, 'Clinical.Outcome'] %in% c('Recovered', 'Deceased'), ]
...
receptor.v <-
  unique(as.vector(apply(cyto[, 2:ncol(cyto)], 2, as.character)))
str(cytokine.v)
str(receptor.v)
...
getcontmat <- function(pred, true) {
  # rows are true, col are pred
  contmat <-
    matrix(0, nrow = length(unique(true)), ncol = length(unique(pred)))
...
leaveid.list1 <-
  split(pbmc.s[id1], ceiling(seq_along(pbmc.s[id1]) / (length(id1) / 10)))
leaveid.list2 <-
  split(pbmc.s[id2], ceiling(seq_along(pbmc.s[id2]) / (length(id2) / 10)))
leaveid.list <- lapply(1:length(leaveid.list1), function(i) {
  c(leaveid.list1[[i]], leaveid.list2[[i]])
})
saveRDS(leaveid.list, paste0(rdir, 'leaveid.list_allSampleNoWindow.rds'))
...
get_fealist <- function(feature.type = 'gene') {
  ...
  for (j in 1:10) {
    set.seed(j)
    leaveid <- leaveid.list[[j]]
    trainid <- setdiff(pbmc.s, leaveid)
    if (length(table(d.pt[leaveid, 'Clinical.Outcome'])) == 2 &
        length(table(d.pt[trainid, 'Clinical.Outcome'])) == 2) {
      data <- lapply(1:length(d), function(cluid) {
        ...
        if (feature.type == 'gene') {
          ...
          mat <-
            mat[rownames(mat) %in% cytokine.v, colnames(mat) %in% pbmc.s, drop = F]
        } else if (feature.type == 'receptor') {
          mat <-
            mat[rownames(mat) %in% receptor.v, colnames(mat) %in% pbmc.s, drop = F]
        } else if (feature.type == 'tf') {
          mat <-
            mat[rownames(mat) %in% tf.v, colnames(mat) %in% pbmc.s, drop = F]
        }
        s.int <- intersect(colnames(mat), pbmc.s)
        mat <- mat[, s.int]
        tmpmat <-
          matrix(NA, nrow = nrow(mat), ncol = length(pbmc.s))  ## missing as NA !!
        dimnames(tmpmat) <- list(rownames(mat), pbmc.s)
        tmpmat[rownames(mat), colnames(mat)] <- mat
        rownames(tmpmat) <-
          paste0('cluster', sub('c', '', names(d)[cluid]), ';', rownames(tmpmat))
        tmpmat
      })
      ...
      tryCatch({
        ...
        model <-
          xgboost(
            data = t(mat[, trainid, drop = FALSE]),
            label = trans(y),
            nrounds = 20,
            objective = 'binary:logistic',
            verbose = F
          )  ###### nrounds to be 20 !!! numclu = 2
        impt <- data.frame(xgb.importance(model = model))
        saveRDS(impt, paste0(rdir, 'impt_all_seed', j, '.rds'))
...
getauc <-
  function(base = TRUE,
    ...
    for (j in 1:length(allfealist)) {
      ...
      fea.type <- NULL
      if (base == TRUE)
        fea.type <- c(fea.type, 'base')
      if (cytokine == TRUE)
        fea.type <- c(fea.type, 'cytokine')
      if (receptor == TRUE)
        fea.type <- c(fea.type, 'receptor')
      if (tf == TRUE)
        fea.type <- c(fea.type, 'tf')
      if (cellProp == TRUE)
        fea.type <- c(fea.type, 'cellProp')
      if (gene == TRUE)  {
        fea.type <- c(fea.type, 'gene')
        fea.type <-
          setdiff(fea.type, c('cytokine', 'receptor', 'tf'))
      }
      mat <- allfealist[[j]]
      g <- sub('.*;', '', rownames(mat))
      ...
      mat <-
        rbind(mat, pseudotime = as.numeric(d.pt[colnames(mat), 'Pseudotime']))
      model <-
        xgboost(
          data = t(mat[, trainid, drop = F]),
          label = trans(y),
          nrounds = 20,
          objective = 'binary:logistic',
          verbose = F
...
saveRDS(fealist.gene, paste0(rdir, 'selected_features_gene_all.rds'))

# ------------------
## select features for cytokine
fealist.cyt <- get_fealist(feature.type = 'cytokine')
saveRDS(fealist.cyt,
        paste0(rdir, 'selected_features_cytokine_all.rds'))

# --------------------
## select features for receptor
fealist.rec <- get_fealist(feature.type = 'receptor')
saveRDS(fealist.rec,
        paste0(rdir, 'selected_features_receptor_all.rds'))

fealist.tf <- get_fealist(feature.type = 'tf')
saveRDS(fealist.tf, paste0(rdir, 'selected_features_tf_all.rds'))

# -----------------------------
## combine selected features of base, gene, cytokine, receptor, tf, cellprop
allfealist <-
  sapply(which(sapply(fealist.gene, length) > 0), function(i) {
    m1 <- fealist.gene[[i]]
    ...
    m4 <- fealist.tf[[i]]
    rownames(m4) <-
      paste0('tf:', rownames(m4))          ################3
    m <-
      rbind(m1, m2, m3, m4, t(fea.base), cellProp.mat[i, pbmc.s])
    rownames(m)[nrow(m)] <- 'cellProp:'
    m
  })
names(allfealist) <-
  names(fealist.gene)[which(sapply(fealist.gene, length) > 0)]
...
saveRDS(allauc, paste0(rdir, 'AUC_allSampleNoWindow.rds'))
...
pd$Type <-
  factor(as.character(pd$Type), levels = names(sort(tapply(
    pd$AUC, list(as.character(pd$Type)), median
  ))))
pdf(
  paste0(pdir, 'AUC_allSampleNoWindow_boxplot.pdf'),
  width = 8,
  height = 4
)
print(

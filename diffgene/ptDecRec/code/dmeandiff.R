...
if (length(group) > 0 && min(table(group)) >= 10) {
  ...
  names(p) <- colnames(m)
  start <- min(p[names(which(group=='Deceased'))])
  
  pval <- mclapply(rownames(m),function(g) {
    set.seed(12345)
    e <- m[g,]
    mean1 <- mean(e[intersect(names(which(p >= start)),names(which(group=='Deceased')))])
    mean2 <- mean(e[intersect(names(which(p >= start)),names(which(group=='Recovered')))])
    e[intersect(names(which(p >= start)),names(which(group=='Deceased')))] <- e[intersect(names(which(p >= start)),names(which(group=='Deceased')))] - mean1 + mean2
    reald <- mgcv::gam(e~s(p,k=3,by=group),data=data.frame(e=e,p=p,group=group))$deviance
  ...
  saveRDS(pval,file=paste0('/home-4/zji4@jhu.edu/scratch/covid/analysis/pbmc/diffgene/withincluster/deceasept/dmeanres/',i,'.rds'))
}
